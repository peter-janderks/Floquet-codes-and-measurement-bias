...
However, to the best of our knowledge, SIGF has previously only been applied to subsystem codes.
Furthermore, it has not previously been studied how well it can handle a noise model biased towards \textit{measurement} errors.
...
Then we present numerical results and see how well our expectations hold up -- again, first for the more tractable case of phenomenological noise, and then the more realistic circuit-level noise.
...
Consider such a logical error in the unrepeated DCCC.
...
We show this in \Cref{fig:repeated_measurements_timelike_distances} -- the topmost subfigure shows the phenomenological noise case.
...
\begin{figure}[p]
    ...
    \caption{
        ...
        But for all other rounds such an error now corresponds to an ordinary egde.
...
\subsection{Phenomenological noise results}\label{subsec:repeating_measurements_phenomenological_noise}

\Cref{fig:phenomenological_noise_volume_plot_pymatching,fig:phenomenological_noise_volume_plot_beliefmatching} show the teraquop volumes of the best $X^a Y^b Z^c$ and $X^a Z^b$ codes under phenomenological noise models of different biases, decoded with MWPM and belief matching respectively.
...
\subsection{Circuit-level noise results}\label{subsec:repeating_measurements_circuit_level_noise}

\Cref{fig:pymatching_circuit_level_noise_volume_plot,fig:beliefmatching_circuit_level_noise_volume_plot} show the teraquop volumes of the best codes under the three circuit-level noise models, decoded with MWPM and belief matching respectively.
...
\begin{figure}[t]
    ...
    \caption{
        Teraquop volumes of the $X^aY^bZ^c$ code for phenomenological noise with measurement bias 16,
...
\begin{figure}[p]
    ...
    \caption{
        Comparison of $\tilde{n}_E, \tilde{n}_M, \tilde{h}_E, \tilde{h}_M,$ and teraquop volume of the $X^1Z^1$ and $X^3Z^3$ code decoded using MWPM and belief matching for unbiased, $Z$-biased, and measurement-biased phenomenological noise.
...
\begin{figure}[p]
    ...
    \caption{
        ...
        Since standard depolarizing noise and superconducting-inspired noise differ only in the probabilites of the certain gates occuring, and not in which gates can occur, any notions of distance are identical across both models.
        ...
        The timelike distances of $X^a Y^b Z^c$ codes are in general reduced under single-qubit measurements noise as compared to phenomenological noise,
