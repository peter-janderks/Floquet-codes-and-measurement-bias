...
\begin{keypoint}\label{key:XYZ_XZ_footprints_similar}
    The differences in teraquop volume between the $X^1 Y^1 Z^1$ and the $X^1 Z^1$ codes are primarily due to differences in the number of rounds of measurements required to reach the teraquop regime -- not in the number of physical qubits (footprint).
    
\end{keypoint}

That is, the differences in footprints $\tilde{n}_M \tilde{n}_E$ across codes and noise models are generally far smaller than the difference in the teraquop heights $\tilde{h}$.
