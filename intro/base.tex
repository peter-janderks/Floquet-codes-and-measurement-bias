...
In parallel with the development of these prototypes, the recent discovery of \textit{dynamic stabilizer codes} has led to new ways of implementing surface code blocks. This started with the introduction of the honeycomb code \cite{hastings2021dynamically}.
...
A logical error rate of $10^{-12}$ would be required for a lattice surgery quantum computer to be able to solve important problems~\cite{lee2021even, gidney2021factor, haner2020improved}.
...
The footprint of what we'll refer to as the $X^1 Y^1 Z^1$ honeycomb code has been studied in \Ccite{gidney2022benchmarking, gidney2021fault, hilaire2024enhanced}.
...
\subsubsection{Phenomenological noise results}\label{subsubsec:phenomenological-noise-results}

\Cref{fig:phenomenological_noise_volume_plot_beliefmatching,fig:phenomenological_noise_volume_plot_pymatching} show the performance of DCCCs for phenomenological noise models decoded using MWPM and belief matching.
...
\subsubsection{Circuit-level noise results}\label{subsubsec:circuit-level-noise-results}

Three circuit-level noise models are simulated: \emph{superconducting-inspired} (SI) noise, \emph{standard depolarizing} (SD) noise, and \emph{entangling
...
\begin{keypoint}
For all three circuit-level noise models, when decoding with MWPM, it is better to only measure $XX$ and $ZZ$, while when decoding with belief matching, it is better to measure $XX$, $YY$, and $ZZ$.
\end{keypoint}
...
\begin{table}[htbp]
    ...
    %  \multicolumn{2}{c}%{\textbf{Section \ref{sec:DCCCs}}} \\
            ...
            \makecell[lt]{Phenomenological\\noise} & A simple noise model in which Pauli errors can occur before measurement rounds with probabilities $p_X$, $p_Y$ and $p_Z$, and measurement errors can occur with probability $p_m$.\\
            ...
            \makecell[lt]{Circuit-level\\noise} & A more accurate noise model in which a $q$-qubit Pauli error can occur with probability $p_g$ after each gate $g$ on $q$ qubits, and measurement errors can occur with probability $p_m$.\\
            \makecell[lt]{Standard\\depolarizing noise} & A circuit-level noise model that assumes multi-qubit measurements are implemented by single-qubit measurements on auxiliary qubits, and all error probabilities are equal.\\
            \makecell[lt]{Superconducting-\\inspired noise} & A circuit-level noise model that assumes multi-qubit measurements are implemented by single-qubit measurements on auxiliary qubits, and the error probabilities model those experienced by real superconducting hardware.\\
            \makecell[lt]{Entangling\\measurement noise} & A circuit-level noise model that assumes multi-qubit measurements are implemented directly, and all error probabilities are equal.\\
            \hline
        \end{tabularx}
    }
    \caption{
